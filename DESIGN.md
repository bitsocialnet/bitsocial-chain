...
Phase 2 of the Bitsocial master plan proposes Bitsocial Network as the economic layer for Bitsocial apps: "unstoppable financial structures, decentralized Bitsocial domains (.bso), awards and tipping, common liquidity, and practical monetization."
...
Phase 1 builds the wedge apps (5chan, Seedit) and the public RPC layer; Phase 2 adds the economic layer so those apps gain network effects that centralized competitors can't choke off; Phase 3+ build the flagship profile-based app on top. This repo is the first Phase-2 artifact: it proves the architecture Bitsocial Network primitives should share — Ethereum-anchored, derivable by anyone, no operator in the trust path — on the smallest useful feature.
...
```
client → BsoNetworkResolver.resolve({ name: "alice.bso" })
